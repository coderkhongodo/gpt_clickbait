...
# Output directory for UIT-VSFC sentiment (3-class) instructions
OUTPUT_DIR = os.environ.get("DATA_DIR", os.path.join("data_processed", "jsonl_text_vsfc_sentiment"))
...
def normalize_label_3class(label: str) -> str:
	# VSFC sentiments.txt already uses 0=neg,1=neutral,2=pos
	# Keep all labels as-is, ensure valid token
	return label.strip()
...
def convert_split(split: str) -> str:
	...
	records: List[dict] = []
	inst = (
		"Bạn là bộ phân loại cảm xúc.\n"
		"Quy ước:\n"
		"- 0 = negative: có ý chê, phàn nàn, bất mãn.\n"
		"- 1 = neutral: trung tính, mô tả/nhận xét bình thường, không khen không chê; lời xã giao (“cảm ơn thầy”) coi là neutral.\n"
		"- 2 = positive: khen ngợi, hài lòng, đánh giá tích cực.\n\n"
		"Chỉ trả đúng MỘT ký tự trong {0,1,2}. Không thêm chữ nào khác.\n\n"
		"Câu cần phân loại:"
	)

	for text, raw_label in zip(texts, labels):
		label3 = normalize_label_3class(raw_label)
		if label3 not in {"0", "1", "2"}:
			continue
		records.append({
			"instruction": inst,
			"input": f"{text}\nĐáp án:",
